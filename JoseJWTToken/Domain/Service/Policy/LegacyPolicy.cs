...
namespace JoseJWTToken.Domain.Service.Policy
{
    ...
    public class LegacyPolicy : IPolicy
    {
        ...
        {
            List<Transport.Domain.AuthPolicy.Location> fences = new List<Transport.Domain.AuthPolicy.Location>();
            List<Transport.Domain.AuthPolicy.TimeFence> timeFences = new List<Transport.Domain.AuthPolicy.TimeFence>();
            foreach (IFence fence in Fences)
            {
                if (fence.GetType() == typeof(GeoCircleFence))
                {
                    fences.Add(new Transport.Domain.AuthPolicy.Location(
            ...
            {
                timeFences.Add(
                    new Transport.Domain.AuthPolicy.TimeFence(
                        name: timeFence.Name,
