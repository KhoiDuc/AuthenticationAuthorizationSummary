...
using JoseJWTToken.Domain.Webhook;
...
namespace JoseJWTToken.Client
{
    public class BasicServiceClient : IServiceClient
    {
        ...
        public string Authorize(string user, string context = null, Domain.Service.AuthPolicy policy = null)
        {
            return CreateAuthorizationRequest(user, context, policy).Id;
        }

        public AuthorizationRequest CreateAuthorizationRequest(string user, string context = null, Domain.Service.AuthPolicy policy = null, string title = null, int? ttl = null, string pushTitle = null, string pushBody = null, IList<Domain.Service.DenialReason> denialReasons = null)
        {
            ...
            List<Transport.Domain.DenialReason> transportDenialReasons;
            ...
            {
                transportDenialReasons = new List<Transport.Domain.DenialReason>();
                foreach (Domain.Service.DenialReason dr in denialReasons)
                {
                    transportDenialReasons.Add(new Transport.Domain.DenialReason(dr.Id, dr.Reason, dr.Fraud));
                }
            }
        ...
        public AuthorizationResponse GetAuthorizationResponse(string authorizationRequestId)
        {
            ...
            {
                Domain.Service.AuthPolicy authPolicy = GetAuthPolicyFromAuthResponsePolicy(advancedAuthResponse.Policy);
        ...
        private AdvancedAuthorizationResponse ParseAuthsGetToAdvancedAuthorizationPackage(ServiceV3AuthsGetResponse response)
        {
            ...
            {
                List<Domain.Service.Policy.IFence> fences = new List<Domain.Service.Policy.IFence>();

                if(response.AuthPolicy.Geofences != null)
                {
                    foreach (Transport.Domain.IFence fence in response.AuthPolicy.Geofences)
                    {
        ...
        private Domain.Service.AuthPolicy GetAuthPolicyFromAuthResponsePolicy(AuthorizationResponsePolicy authResponsePolicy)
        {
            Domain.Service.AuthPolicy authPolicy = new Domain.Service.AuthPolicy();
            ...
            if (authResponsePolicy != null)
            {
                ...
                {
                    foreach (Domain.Service.Policy.IFence fence in authResponsePolicy.Fences)
                    {
                        if(fence.GetType() == typeof(Domain.Service.Policy.GeoCircleFence))
                        {
                            authPolicyLocations.Add(
                                new Location(
                                    (fence as Domain.Service.Policy.GeoCircleFence).Radius,
                                    (fence as Domain.Service.Policy.GeoCircleFence).Latitude,
                                    (fence as Domain.Service.Policy.GeoCircleFence).Longitude,
                                    (fence as Domain.Service.Policy.GeoCircleFence)?.Name
                                )
                            );
                        }
                        else
                ...
                {
                    authPolicy = new Domain.Service.AuthPolicy(
        ...
        public IWebhookPackage HandleWebhook(Dictionary<string, List<string>> headers, string body, string method = null, string path = null)
        {
            ...
            else if(webhookPackage is AdvancedAuthorizationResponseWebhookPackage)
            {
                ...
                Domain.Service.AuthPolicy authPolicy = GetAuthPolicyFromAuthResponsePolicy(advancedAuthorizationResponse.Policy);
